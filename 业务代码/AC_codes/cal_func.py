...
def calculate_it(air_dict):
    ...
    ser_p = dict_read[9]
    por = dict_read[11] if air_dict['isFC'] else float(round(load_time / run_time, 4))

    # 空载浪费
    d_val = dict_read[10]

    # 空载浪费
    empty_waste = 0 if air_dict['isFC'] else round((run_time - load_time) / run_time * 0.4 * ori_power, 4)
    str_empty_waste = '0' if air_dict['isFC'] else '(' + str(run_time) + '-' + str(load_time) + ')' + '/' + str(
        run_time) + '*40%' + '*' + str(ori_power) + '=' + str(empty_waste)
    ...
    actual_energyE = round((float(ori_power) * ser_p * d_cal_wast_por) / (air * por), 2) if air_dict['isFC'] else round(
    ...
    str_actual_energyE = '(' + str(ori_power) + '*' + str(ser_p) + '*' + str(d_cal_wast_por) + ')/(' + str(
        air) + '*' + str(por) + ')=' + str(actual_energyE) if air_dict['isFC'] else '(' + str(ori_power) + '*' + str(
        ser_p) + '*' + str(por) + '+' + str(
...
def originEC_to_dataframe(machine1, new_machine):
    ...
    ee_dicts = []
    for one in machine1:
        data_list, eq_list, ee_list = calculate_it(one)
